...
function circleFillCommand(cx: number, cy: number, radius: number, fill: Rgb): string {
  const c = radius * 0.5522847498
  const path = [
    `${n(cx + radius)} ${n(cy)} m`,
    `${n(cx + radius)} ${n(cy + c)} ${n(cx + c)} ${n(cy + radius)} ${n(cx)} ${n(cy + radius)} c`,
...
function polygonFillCommand(points: Array<{ x: number; y: number }>, fill: Rgb): string {
