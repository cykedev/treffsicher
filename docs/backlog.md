# Backlog — Treffsicher (Stand 2026-03-06)

Priorisierter Themenkatalog nach vollständiger Code-Analyse.
...
**Betroffen:**
- `location`, `trainingGoal` in `CreateSessionSchema` (`shared.ts:12-18`)
- `reflection.observations`, `.insight`, `.learningQuestion`, `.routineDeviation`
- `feedback.explanation`, `.wentWell`, `.insights`, `.progress`, `.fiveBestShots`, `.goalAchievedNote`
- `prognosis.expectedScore`, `.performanceGoal`

**Fix:** `.max(500)` auf kurze Felder, `.max(2000)` auf Langtext-Felder
**Aufwand:** Klein (~30 min)
**Dateien:** `src/lib/sessions/actions/shared.ts`, `src/lib/sessions/actions/mentalActions.ts`
**Spec:** @docs/technical-constraints.md#daten--und-aktionsarchitektur (Zod-Validierung in Server Actions)
...
**Fix:** `parseDimensionsFromFormData()` extrahieren, beide Actions nutzen diese Funktion.
...
**Problem:** Schemas, Typen, Hilfsfunktionen und Parsing-Logik gemischt.
**Fix:** Schema-Definitionen in `sessionSchemas.ts` auslagern; `shared.ts` behält nur Hilfsfunktionen.
...
**Fix:** Tab-State in eigenen Hook, Chart-Tab-Router als dünne Orchestrator-Komponente.
**Aufwand:** Mittel (~1 h)
...
**Problem:** Kein `error.tsx` in `/(app)/`. Client-Fehler crashen die gesamte App-Shell.
**Fix:** `src/app/(app)/error.tsx` + `src/app/(app)/sessions/[id]/error.tsx`
**Aufwand:** Klein (~30 min)
...
**Scope:** `next-pwa`, App-Manifest, Offline-Fallback-Seite
**Aufwand:** Groß (~1 Tag)
**Spec:** @docs/requirements.md#offene-punkte-spätere-phasen

### T-07: `trainingGoal`-Textarea — `maxLength`-Attribut im UI
**Problem:** Textarea hat kein `maxLength`, Limit wird nur serverseitig enforced (sobald T-01 umgesetzt).
**Fix:** `maxLength` + Zeichen-Zähler im `EinheitForm`
**Aufwand:** Sehr klein (~15 min)
**Abhängigkeit:** T-01
**Spec:** @docs/requirements.md#die-einheit--herzstück-des-systems (optionale Basisfelder: trainingGoal)
